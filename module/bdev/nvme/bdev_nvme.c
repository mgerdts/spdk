...
struct nvme_ctrlr *
nvme_bdev_ctrlr_get_ctrlr_by_subnqn(struct nvme_bdev_ctrlr *nbdev_ctrlr,
				    const char *subnqn)
{
	struct nvme_ctrlr *nvme_ctrlr;

	TAILQ_FOREACH(nvme_ctrlr, &nbdev_ctrlr->ctrlrs, tailq) {
		if (strcmp(subnqn, nvme_ctrlr->active_path_id->trid.subnqn) == 0) {
			break;
		}
	}
 ...
 * Hence check only the multipath capability here.
 */
static bool
			     ...
			     struct spdk_nvme_transport_id *trid)
{
	...
	rc = bdev_nvme_check_secondary_trid(nvme_ctrlr, trid);
	if (rc != 0) {
		goto exit;
	}
		 ...
		 bool multipath)
{
	...
	ctx->probe_ctx = spdk_nvme_connect_async(trid, &ctx->drv_opts, connect_attach_cb);
