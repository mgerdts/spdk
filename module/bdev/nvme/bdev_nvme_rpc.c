...
static int
rpc_bdev_nvme_populate_trid(struct spdk_nvme_transport_id *trid,
			    struct rpc_bdev_nvme_attach_controller *req)
{
	size_t len, maxlen;
	int rc;
	...
	if (len == maxlen) {
		SPDK_ERRLOG("traddr was too long: %s\n", req->traddr);
		return -EINVAL;
	}
	...
	if (req->trsvcid) {
		...
		if (len == maxlen) {
			SPDK_ERRLOG("trsvcid was too long: %s\n", req->trsvcid);
			return -EINVAL;
		}
	...
	if (req->priority) {
		trid->priority = spdk_strtol(req->priority, 10);
	}
	...
	if (req->subnqn) {
		...
		if (len == maxlen) {
			SPDK_ERRLOG("subnqn was too long: %s\n", req->subnqn);
			return -EINVAL;
		}
				...
				const struct spdk_json_val *params)
{
	...
	rc = rpc_bdev_nvme_populate_trid(&trid, &ctx->req);
	if (rc != 0) {
		spdk_jsonrpc_send_error_response(request, rc, spdk_strerror(-rc));
		goto cleanup;
	}
	...
	if (ctx->req.hostaddr) {
		...
		if (len == maxlen) {
			SPDK_ERRLOG("hostaddr was too long: %s\n", ctx->req.hostaddr);
	...
	if (ctx->req.hostsvcid) {
		...
		if (len == maxlen) {
			SPDK_ERRLOG("hostsvcid was too long: %s\n", ctx->req.hostsvcid);
			spdk_jsonrpc_send_error_response(request, -EINVAL, spdk_strerror(EINVAL));
			goto cleanup;
		}
	...
	if (ctrlr) {
		...
		if (ctx->req.multipath == BDEV_NVME_MP_MODE_DISABLE) {
			...
			SPDK_ERRLOG("A controller named %s already exists and multipath is disabled\n",
				    ctx->req.name);
			spdk_jsonrpc_send_error_response(request, -EALREADY, spdk_strerror(EALREADY));
			goto cleanup;
		}
		...
		if (ctx->req.bdev_opts.prchk_flags) {
			SPDK_ERRLOG("A controller named %s already exists. To add a path, do not specify PI options.\n",
				    ctx->req.name);
			spdk_jsonrpc_send_error_response(request, -EINVAL, spdk_strerror(EINVAL));
			goto cleanup;
		}
	...
	if (ctx->req.drv_opts.num_io_queues == 0 || ctx->req.drv_opts.num_io_queues > UINT16_MAX + 1) {
		SPDK_ERRLOG("num_io_queues out of bounds, min: %u max: %u\n", 1, UINT16_MAX + 1);
		spdk_jsonrpc_send_error_response(request, -EINVAL, spdk_strerror(EINVAL));
		goto cleanup;
	}
