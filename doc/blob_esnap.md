...
with the appropriate blob using `spdk_blob_set_esnap_bs_dev()`.
...
functions, there is no way to determine which channel should be used. Thus, it will be up to the
callers to provide the right channel for IO on the back device (blob or esnap) via
`set->back_channel`.
...
lvs_uuid                | Required | string      | UUID of logical volume store to create logical volume on
