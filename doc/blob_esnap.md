...
Much like with regular clones, an external clone may read clusters that are allocated to the clone
...
external clone on blocks that are part of the blobstore's device (`blob->bs->dev`).
...
When an external clone is created, `external_snapshot_id` and `external_snapshot_id_len` are
set in the options passed to `spdk_bs_create_blob_ext()`. This identifier is stored in the
`BLOB_EXTERNAL_SNAPSHOT_ID` internal XATTR. This identifier serves must be usable for the life
of the blob, including across restarts of the SPDK application.
...
`spdk_blob_get_external_id()` to get the external identifier and may use
...
- As an SPDK application starts, the initialization of bdevs can happen in any order.
- External clone blobs can be opened so that IO that is not dependent on the external snapshot
  device can be serviced.
- bdevs that depend upon the external clone blob can be opened, which reserves their name and UUID
  in the global namespaces.
...
memory references it obtained from `spdk_blob_get_external_id()`, `spdk_blob_get_xattr_value()`,
...
1. An internal XATTR with name `BLOB_EXTERNAL_SNAPSHOT_ID` ("EXTSNAP") exists.
...
Clones of external snapshots are created by setting `external_snapshot_id` and
`external_snapshot_id_len` in `struct spdk_blob_opts` while calling `spdk_bs_create_blob_ext()`.

```c
struct spdk_blob_opts {
	...
        /**
	 * If set, pass this identifier to bs->external_bs_dev_create() while creating an external
	 ...
	 * this same bdev when the blob is loaded.
         */
        void *external_snapshot_id;

	/**
	 * The size of external_snapshot_id, in bytes.
	 */
	uint32_t external_snapshot_id_len;
};
```
...
struct spdk_bs_opts {
	 ...
	 * spdk_blob_get_external_id(), spdk_blob_get_id(), etc. After calling cb(), it is not
