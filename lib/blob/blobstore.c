...
#define SET_FIELD(field, value) \
	...
	if (FIELD_OK(external_snapshot_name)) {
		opts->external_snapshot_name = NULL;
	}
...
blob_load_backing_dev(void *cb_arg)
{
	...
	if (spdk_blob_is_external_clone(blob)) {
		...
		name = spdk_blob_get_external_parent(blob);
		if (name == NULL) {
		...
		blob_create_esnap_dev(blob->bs, blob, name, blob_esnap_load_done, ctx);
		return;
	}
...
#define SET_FIELD(field) \
	...
	if (FIELD_OK(external_snapshot_name)) {
		dst->external_snapshot_name = src->external_snapshot_name;
	}
	       ...
	       spdk_blob_op_with_id_complete cb_fn, void *cb_arg)
{
	...
	if (opts_local.external_snapshot_name != NULL) {
		char *name = opts_local.external_snapshot_name;
		struct spdk_bdev *parent;

		parent = spdk_bdev_get_by_name(name);
		if (parent == NULL) {
		...
		if (spdk_bdev_get_block_size(parent) > blob->bs->io_unit_size) {
			 ...
			 */
			SPDK_ERRLOG("External snapshot device %s (%s) block "
				    "size %" PRIu32 " is larger than blobstore "
				    "io_unit_size %" PRIu32 "\n",
				    spdk_bdev_get_name(parent), name,
		...
		rc = blob_set_xattr(blob, BLOB_EXTERNAL_SNAPSHOT_BDEV, name,
				    strlen(name) + 1, true);
		if (rc != 0) {
			blob_free(blob);
			spdk_bit_array_clear(bs->used_blobids, page_idx);
