...
bs_claim_md_page(struct spdk_blob_store *bs, uint32_t page)
{
	...
	assert(spdk_spin_held(&bs->used_clusters_lock));
		    ...
		    uint64_t *cluster, uint32_t *lowest_free_md_page, bool update_map)
{
	...
	assert(spdk_spin_held(&blob->bs->used_clusters_lock));
...
blob_back_bs_destroy_esnap_done(void *ctx, struct spdk_blob *blob, int bserrno)
{
	...
	if (bserrno != 0) {
		 ...
		 * blob_esnap_destroy_ctx before iterating threads. This should be handled better.
		 */
		abort();
	}
...
blob_resize(struct spdk_blob *blob, uint64_t sz)
{
	 ...
	 * bs->used_cluster_lock is held to ensure that clusters we think are free are still free
	 * when we go to claim them later in this function.
	 */
	...
	rc = 0;
out:
	if (spdk_spin_held(&bs->used_clusters_lock)) {
		spdk_spin_unlock(&bs->used_clusters_lock);
	}
...
blob_persist_generate_new_md(struct spdk_blob_persist_ctx *ctx)
{
	 ...
	 * enough pages and a second to actually claim them. The used_clusters_lock is held across
			     ...
			     spdk_blob_op_complete cb_fn, void *cb_arg, enum spdk_blob_op_type op_type)
{
	...
	seq = bs_sequence_start_external(ch, &cpl, blob);
	if (!seq) {
		free(ctx);
		cb_fn(cb_arg, -ENOMEM);
		return;
			   ...
			   struct spdk_blob_ext_io_opts *ext_io_opts)
{
	...
	if (spdk_likely(length <= bs_num_io_units_to_cluster_boundary(blob, offset))) {
		...
		if (read) {
			...
			seq = bs_sequence_start_external(_channel, &cpl, blob);
			if (!seq) {
				cb_fn(cb_arg, -ENOMEM);
				return;
			}
...
bs_load_replay_md_cpl(spdk_bs_sequence_t *seq, void *cb_arg, int bserrno)
{
	...
	if (bs_load_cur_md_page_valid(ctx) == true) {
		if (page->sequence_num == 0 || ctx->in_page_chain == true) {
			spdk_spin_lock(&ctx->bs->used_clusters_lock);
			bs_claim_md_page(ctx->bs, page_num);
			spdk_spin_unlock(&ctx->bs->used_clusters_lock);
			if (page->sequence_num == 0) {
...
static int
bs_snapshot_copy_xattr(struct spdk_blob *fromblob, struct spdk_blob *toblob,
		       const char *name)
{
	const void	*val = NULL;
	size_t		len;
	...
	if (bserrno != 0) {
		SPDK_ERRLOG("blob 0x%" PRIx64 " missing %s xattr"
			    BLOB_EXTERNAL_SNAPSHOT_COOKIE " XATTR\n", fromblob->id, name);
		return bserrno;
	}
...
bs_snapshot_origblob_sync_cpl(void *cb_arg, int bserrno)
{
	...
	if (bserrno != 0) {
		...
		if (newblob->invalid_flags & SPDK_BLOB_EXTERNAL_SNAPSHOT) {
			bs_snapshot_copy_xattr(newblob, origblob, BLOB_EXTERNAL_SNAPSHOT_COOKIE);
...
bs_snapshot_newblob_sync_cpl(void *cb_arg, int bserrno)
{
	...
	if (origblob->invalid_flags & SPDK_BLOB_EXTERNAL_SNAPSHOT) {
		origblob->invalid_flags &= ~SPDK_BLOB_EXTERNAL_SNAPSHOT;
		bserrno = blob_remove_xattr(origblob, BLOB_EXTERNAL_SNAPSHOT_COOKIE, true);
		if (bserrno != 0) {
			if (bserrno == -ENOENT) {
				SPDK_NOTICELOG("blob 0x%" PRIx64 " has no "
					       BLOB_EXTERNAL_SNAPSHOT_COOKIE " xattr to remove\n",
					       origblob->id);
			} else {
				/* return cluster map back to original */
				bs_snapshot_swap_cluster_maps(newblob, origblob);
...
bs_snapshot_freeze_cpl(void *cb_arg, int rc)
{
	...
	case SPDK_BLOBID_EXTERNAL_SNAPSHOT:
		bserrno = bs_snapshot_copy_xattr(origblob, newblob, BLOB_EXTERNAL_SNAPSHOT_COOKIE);
		if (bserrno != 0) {
			bs_clone_snapshot_newblob_cleanup(ctx, bserrno);
			return;
		}
...
bs_snapshot_origblob_open_cpl(void *cb_arg, struct spdk_blob *_blob, int bserrno)
{
	...
	if (_blob->data_ro || _blob->md_ro) {
		SPDK_DEBUGLOG(blob, "Cannot create snapshot from read only blob with id 0x%" PRIx64
			      "\n", _blob->id);
		ctx->bserrno = -EINVAL;
...
bs_inflate_blob_done(struct spdk_clone_snapshot_ctx *ctx)
{
	...
	if (ctx->allocate_all) {
		...
		if (_blob->parent_id == SPDK_BLOBID_EXTERNAL_SNAPSHOT) {
			blob_remove_xattr(_blob, BLOB_EXTERNAL_SNAPSHOT_COOKIE, true);
...
delete_snapshot_update_extent_pages_cpl(struct delete_snapshot_ctx *ctx)
{
	...
	if (ctx->snapshot->parent_id == SPDK_BLOBID_EXTERNAL_SNAPSHOT) {
		bserrno = bs_snapshot_copy_xattr(ctx->snapshot, ctx->clone,
						 BLOB_EXTERNAL_SNAPSHOT_COOKIE);
		if (bserrno != 0) {
			ctx->bserrno = bserrno;
...
{
	assert(blob != NULL);
	return blob_is_external_clone(blob);
}
...
/* XXX-mg reconsider the allocation here. So far the callers just free it immediately. */
int
...
blob_esnap_get_io_channel(struct spdk_io_channel *ch, struct spdk_blob *blob)
{
	...
	SPDK_DEBUGLOG(blob_esnap, "blob 0x%" PRIx64 ": allocated channel on thread %s\n",
		      blob->id, spdk_thread_get_name(spdk_get_thread()));
...
/*
 * Destroy the all bs_dev channels on a specific blobstore channel. This should be used when a
