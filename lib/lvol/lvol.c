 ...
 * loaded.
 *
 * As an external clone blob opens, the blobstore calls bs->external_bs_dev_create(), which for an
 * lvstore is lvs_esnap_dev_create(). It is important that any failure to open the external snapshot
 * during this first pass does not prevent the lvol from being added to lvs->lvols, as that would
 * leave an lvol with no name or UUID lvs->pending_lvols. An anonymous pending lvol such as this
 * would not serve the purpose of reserving a name or UUID, nor would it allow it to be deleted or
 * otherwise repaired. Thus, it is important that lvs_esnap_dev_create() returns success while
 * lvs->load_esnaps is false.
 *
