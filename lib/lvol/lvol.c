...
load_next_lvol(void *cb_arg, struct spdk_blob *blob, int lvolerrno)
{
	...
	if (esnap_name != NULL) {
		/* XXX-mg Is there strdup needed?  I'm guessing so since we don't free esnap_name. */
		lvol->esnap_name = strdup(esnap_name);
			    ...
			    spdk_lvol_op_with_handle_complete cb_fn, void *cb_arg)
{
	...
	spdk_blob_opts_init(&opts, sizeof(opts));
	opts.external_snapshot_name = lvol->esnap_name;
