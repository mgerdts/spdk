...
lvol_esnap_degraded(void)
{
	...
	/* Creation of a degraded device fails gracefully when the lvol can't be found */
	rc = lvs_esnap_dev_create_degraded(lvs, &blob, name, &bs_dev);
	CU_ASSERT(rc == -EINVAL);

	/* Creation succeeds when the lvol can be found */
	bs_dev = NULL;
	TAILQ_INSERT_TAIL(&lvs->lvols, lvol, link);
	rc = lvs_esnap_dev_create_degraded(lvs, &blob, name, &bs_dev);
	CU_ASSERT(rc == 0);
	SPDK_CU_ASSERT_FATAL(bs_dev != NULL);
	ddev = (struct spdk_lvs_degraded_dev *)bs_dev;
