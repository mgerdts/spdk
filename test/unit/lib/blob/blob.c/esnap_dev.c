...
/*
 * When reading, each block is filled with 64-bit values made up of the least signficant 32 bits of
...
ut_esnap_io_channel_create(void *io_device, void *ctx)
{
	...
	rc = pthread_mutex_lock(&ut_dev->mutex);
	if (rc != 0) {
		CU_ASSERT(false);
		return -rc;
	}
	...
	rc = pthread_mutex_unlock(&ut_dev->mutex);
	if (rc != 0) {
		CU_ASSERT(false);
		return -rc;
	}
...
ut_esnap_dev_free(void *io_device)
{
	...
	if (pthread_mutex_lock(&ut_dev->mutex) != 0) {
		CU_ASSERT(false);
	}
	CU_ASSERT(ut_dev->num_channels == 0);
...
static void
ut_esnap_create(struct spdk_blob *blob, spdk_blob_op_with_bs_dev cb, void *cb_arg)
{
