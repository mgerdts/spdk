...
static bool
is_ext_clone(struct spdk_blob *_blob, const void *cookie, size_t cookie_len)
{
	const void *val = NULL;
	size_t len;
	bool c0, c1, c2, c3;

	CU_ASSERT(blob_get_xattr_value(_blob, BLOB_EXTERNAL_SNAPSHOT_COOKIE, &val, &len,
				       true) == 0);
	CU_ASSERT((c0 = (len == cookie_len)));
	CU_ASSERT((c1 = (val != NULL && memcmp(val, cookie, len) == 0)));
...
is_not_ext_clone(struct spdk_blob *_blob)
{
	...
	CU_ASSERT((c1 = (blob_get_xattr_value(_blob, BLOB_EXTERNAL_SNAPSHOT_COOKIE, &val, &len,
					      true) == -ENOENT)));
...
#define UT_ASSERT_IS_EXT_CLONE(_blob, _cookie, _len) CU_ASSERT(is_ext_clone(_blob, _cookie, _len))
...
blob_bdev_rw(void)
{
	...
	/* XXX-mg transition away from malloc */
...
blob_esnap_create(void)
{
	...
	/* Create an external clone blob and verify it is an esnap and has the right size */
	ut_spdk_blob_opts_init(&opts);
...
blob_extclone_reload(void)
{
	...
	/* Create and open an external clone blob */
	ut_spdk_blob_opts_init(&opts);
	ut_esnap_opts_init(block_sz, esnap_num_blocks, __func__, NULL, &esnap_opts);
	opts.external_snapshot_cookie = &esnap_opts;
	opts.external_snapshot_cookie_len = sizeof(esnap_opts);
	...
	CU_ASSERT(spdk_blob_is_external_clone(eclone1));
	...
	bserr3 = 0xbad;
	bserr4 = 0xbad;
	spdk_blob_close(eclone1, save_errno_cb, &bserr1);
	spdk_blob_close(snap1, save_errno_cb, &bserr2);
	spdk_blob_close(clone1, save_errno_cb, &bserr3);
	spdk_bs_unload(bs, save_errno_cb, &bserr4);
...
blob_esnap_io_size(uint32_t bs_blksz, uint32_t esnap_blksz)
{
	...
	bsopts.external_bs_dev_create = ut_esnap_create;
	...
	/* Create and open the external clone  */
	ut_spdk_blob_opts_init(&opts);
...
blob_esnap_io_512_4096(void)
{
	...
	/* Try to create and open the external clone. Create should succeed, open should fail. */
	ut_spdk_blob_opts_init(&blob_opts);
	ut_esnap_opts_init(esnap_blksz, esnap_num_blocks, __func__, NULL, &esnap_opts);
...
blob_esnap_thread_add_remove(void)
{
	...
	ut_spdk_blob_opts_init(&opts);
	opts.external_snapshot_cookie = &ut_esnap_opts;
	opts.external_snapshot_cookie_len = sizeof(ut_esnap_opts);
	opts.num_clusters = 10;
	spdk_bs_create_blob_ext(bs, &opts, blob_op_with_id_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == 0);
	...
	int blob_close_errno = 0xbad;
	spdk_blob_close(blob, save_errno_cb, &blob_close_errno);
	poll_threads();
	CU_ASSERT(blob_close_errno == 0);
	CU_ASSERT(destroyed);
...
blob_extclone_snapshot(void)
{
	 ...
	 *
	 *   - XATTR_INTERNAL for BLOB_EXTERNAL_SNAPSHOT_COOKIE (e.g. name or UUID)
	...
	UT_ASSERT_IS_EXT_CLONE(snap_blob, &esnap_opts, sizeof(esnap_opts));
...
blob_esnap_hotplug(void)
{
	...
	opts.external_snapshot_cookie = &esnap1_opts;
	opts.external_snapshot_cookie_len = sizeof(esnap1_opts);
	opts.num_clusters = esnap_num_clusters;
	blob = ut_blob_create_and_open(bs, &opts);
	CU_ASSERT(blob != NULL);
	CU_ASSERT(spdk_blob_is_external_clone(blob));
...
main(int argc, char **argv)
{
	...
	CU_ADD_TEST(suite_esnap_bs, blob_extclone_reload);
