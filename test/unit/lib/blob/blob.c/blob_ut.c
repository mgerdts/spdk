...
blob_extclone_defaults(void)
{
	...
	ut_spdk_blob_opts_init(&opts);
	opts.external_snapshot_name = mdisks[0].uuid_str;
...
blob_extclone_size(void)
{
	...
	for (size_t i = 0; i < SPDK_COUNTOF(mdisks); i++) {
		...
		ut_spdk_blob_opts_init(&opts);
		opts.external_snapshot_name = mdisks[i].uuid_str;
	...
	ut_spdk_blob_opts_init(&opts);
	opts.external_snapshot_name = bdev->name;
	spdk_bs_create_blob_ext(bs, &opts, blob_op_with_id_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == 0);
	CU_ASSERT(g_blobid != SPDK_BLOBID_INVALID);
	...
	ut_blob_close_and_delete(bs, blob);
	delete_malloc_disk(bdev->name, bs_op_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == 0);
}
