...
function test_esnap_reload_missing() {
	...
	# State:
	#   esnap(missing) <-- eclone(unhealthy)
	bs_dev=$(rpc_cmd bdev_aio_create "$testdir/aio_bdev_0" "$aio_bdev" "$block_size")
	...
	# State:
	#   esnap(missing) <-- eclone(unhealthy) <-- clone(unhealthy)
	...
	# Reload the lvstore with esnap present during load. This should make the lvols healthy.
	# State:
	#   esnap(present) <-- eclone(healthy) <-- clone(healthy)
	rpc_cmd bdev_aio_delete "$aio_bdev"
	NOT rpc_cmd bdev_lvol_get_lvstores -l lvs_test
	esnap_dev=$(rpc_cmd bdev_malloc_create -u "$esnap_uuid" "$esnap_size_mb" "$block_size")
	bs_dev=$(rpc_cmd bdev_aio_create "$testdir/aio_bdev_0" "$aio_bdev" "$block_size")
	...
	# State:
	#   esnap(missing) <-- eclone(unhealthy) <-- snap(unhealthy) <-- clone(unhealthy)
	...
	# Create the esnap bdev and verify the unhealthy bdevs become healthy.
	esnap_dev=$(rpc_cmd bdev_malloc_create -u "$esnap_uuid" "$esnap_size_mb" "$block_size")
...
function log_jq_out() {
	...
	printf "%s\n" "${!jq_out[@]}" | sort | while read -r key; do
		printf "%50s = %s\n" "$key" "${jq_out[$key]}"
	done
...
function verify_clone() {
	...
	test "${jq_out["supported_io_types.read"]}" == true
	test "${jq_out["supported_io_types.write"]}" == true
	test "${jq_out["driver_specific.lvol.clone"]}" == true
	test "${jq_out["driver_specific.lvol.base_snapshot"]}" == "$parent"
	test "${jq_out["driver_specific.lvol.esnap_clone"]}" == false
	test "${jq_out["driver_specific.lvol.external_snapshot_name"]}" == null
}
...
function test_esnap_clones() {
	...
	verify_esnap_clone "$vol2_uuid" "$esnap_uuid" ro
	...
	verify_esnap_clone "$vol3_uuid" "$esnap_uuid" ro
...
function test_esnap_remove_unhealthy() {
	local bs_dev esnap_dev
	local block_size=512
	local esnap_size_mb=1
	local lvs_cluster_size=$((16 * 1024))
...
run_test "test_esnap_remove_unhealthy" test_esnap_remove_unhealthy
